...
Paths (relative to repo root)
    ...
    INPUT FILE: data/clinic_data_abdellah.csv
    OUTPUT FILE: artifacts/anomalies_abdellah.csv

Terminal command to run this file from the root project folder

    uv run python -m cintel.anomaly_detector_abdellah
...
DATA_FILE: Final[Path] = DATA_DIR / "clinic_data_abdellah.csv"
OUTPUT_FILE: Final[Path] = ARTIFACTS_DIR / "anomalies_abdellah.csv"
...
def main() -> None:
    ...
    # Add a new column to explain the type of anomaly
    df = df.with_columns(
        pl.when(pl.col("height_inches") >= MAX_REASONABLE_Y_VALUE)
        .then(pl.lit("height"))
